...
async function getRepoViews(repos, queries) {
  ...
  for (const repo of repos) {
    try {
      ...
      if (r.uniques && r.uniques > 0 && Array.isArray(r.views) && r.views.length) {
...
<svg width="385" height="180" xmlns="http://www.w3.org/2000/svg">
  <style>
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .title-cards { fill: ${colors.dark.titleCards}; }
      .github-icon path { fill: ${colors.dark.iconGithub}; }
      th {
      ...
      .repo-name { color: ${colors.dark.repositoryText}; }
      .unique-count { color: ${colors.dark.uniqueCount}; }
      .date-range { color: ${colors.dark.dateRange}; }
    }
  </style>
  <g>
