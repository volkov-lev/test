...
function generateSVG(stats) {
  const statRows = [
    ...
    },
    {
      ...
      value: stats.linesChanged,
      icon: `<svg class="octicon" viewBox="0 0 16 16" width="16" height="16"><path fill-rule="evenodd" d="M8.75 1.75a.75.75 0 00-1.5 0V5H4a.75.75 0 000 1.5h3.25v3.25a.75.75 0 001.5 0V6.5H12A.75.75 0 0012 5H8.75V1.75zM4 13a.75.75 0 000 1.5h8a.75.75 0 100-1.5H4z"></path></svg>`
    },
    {
  ...
  // SVG row height and vertical spacing
  const rowHeight = 26;
  const startY = 58;
...
<svg width="360" height="210" xmlns="http://www.w3.org/2000/svg">
  <style>
    ...
    .stat-row {
      ...
      animation: fadeInStat 0.8s cubic-bezier(.33,1.53,.53,1.01) forwards;
    }
    ${statRows.map((_, i) => `.stat-row-${i} { animation-delay: ${0.18 * i + 0.18}s; }`).join("\n")}
    ...
    @media (prefers-color-scheme: dark) {
      .title { fill: ${colors.dark.title}; }
      .label, .value { fill: ${colors.dark.textPrimary}; }
      .icon { fill: ${colors.dark.icon}; }
      rect#background { fill: ${colors.dark.background}; stroke: ${colors.dark.outline}; }
    }
  </style>
  <rect id="background" x="5" y="5" width="350" height="200"/>
  <text x="30" y="37" class="title">${stats.name}'s GitHub Statistics</text>
  ${statRows.map((row, i) => `
    <g class="stat-row stat-row-${i}" style="animation-delay: ${0.18 * i + 0.18}s;">
      <g transform="translate(30,${startY + i * rowHeight})">
        <g>${row.icon}</g>
        <text x="22" y="12" class="label">${row.label}</text>
        <text x="270" y="12" class="value" text-anchor="end">${row.value}</text>
      </g>
    </g>
  `).join("")}
</svg>
